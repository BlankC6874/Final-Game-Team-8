import { GameDSL, parseYAML } from "./yamlParser.ts";
...
function advanceTurn() {
  for (let y = 0; y < gridSize; y++) {
    for (let x = 0; x < gridSize; x++) {
      ...
      gridState[index + 1] = Math.min(
        gridState[index + 1] + (Math.random() > 0.8 ? 1 : 0),
        5,
      );
...
function sowPlant() {
  interactWithCell((index) => {
    if (gridState[index + 2] === 0) {
      gridState[index + 2] =
        Math.floor(Math.random() * availablePlants.length) + 1; // Plant type
